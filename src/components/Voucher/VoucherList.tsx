...
const VoucherList: React.FC<VoucherListProps> = ({ searchTerm, statusFilter }) => {
  ...
  const {
    ...
    url: "/payment/vouchers/admin/all-vouchers?page=0&size=100",
  });
  ...
  useEffect(() => {
    setCurrentPage(1); // reset về page 1 khi search term hoặc status filter thay đổi
              ...
              {paginatedVouchers.map((v, index) => (
                <tr key={v.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                  ...
                  <td className="px-4 py-2 border-b">
                    <span className={classNames("px-2 py-1 text-xs font-semibold rounded-full", statusColorMap[v.status] || "bg-gray-100 text-gray-700")}>{v.status}</span>
                  </td>
                  <td className="px-4 py-2 border-b">
