...
const navItems: NavItem[] = [
  ...
  // },
  {
    name: "🅿️ Parking & Charging station",
    path: "/manage-parking-charging",
  },
  {
    name: "🚗 Lots",
    path: "/slot-list",
...
const othersItems: NavItem[] = [
  {
    name: "Authentication",
    subItems: [{ name: "Sign In", path: "/", pro: false }],
  },
];
...
const AppSidebar: React.FC = () => {
  ...
  useEffect(() => {
    const fetchComplaintCount = async () => {
      try {
        ...
        // Đếm số complaint có status là 'PENDING'
  ...
  // Kiểm tra role trong localStorage và ẩn mục "Customer" nếu không phải là ADMIN
  const userRole = localStorage.getItem("userRole");
  const filteredNavItems = userRole === "ADMIN" ? navItems : navItems.filter((item) => item.name !== "👤 Customer");

  // Thay đổi từ navItems sang filteredNavItems đã lọc
  const computedNavItems: NavItem[] = filteredNavItems.map((item) => {
